...
import { VideoSourceType, VideoProvider, TranscriptStatus } from "@/lib/types";
          ...
          <div className="relative aspect-video mt-2">
            {initialData.videoSourceType === VideoSourceType.UPLOAD ? (
              initialData?.muxData?.playbackId ? (
                ...
                <div className="w-full h-full bg-slate-900 rounded-md flex items-center justify-center text-slate-300 text-sm">
              ...
              <div className="w-full h-full bg-slate-900 rounded-md flex flex-col items-center justify-center text-white p-4 text-center gap-y-2">
                ...
                <Badge
                  variant="outline"
                  className="bg-emerald-500/10 text-emerald-500 border-none mt-2"
                >
                ...
                {initialData.transcriptStatus === TranscriptStatus.PROCESSING && (
                  <Badge
                    variant="outline"
                    className="bg-amber-500/10 text-amber-500 border-none mt-1"
                  >
                ...
                {initialData.transcriptStatus === TranscriptStatus.COMPLETED && (
                  <Badge
                    variant="outline"
                    className="bg-sky-500/10 text-sky-500 border-none mt-1"
                  >
                    <CheckCircle className="h-3 w-3 mr-1" /> Legenda Automática
                    OK
                  </Badge>
                )}
                {initialData.transcriptStatus === TranscriptStatus.FAILED && (
                  <Badge
                    variant="outline"
                    className="bg-rose-500/10 text-rose-500 border-none mt-1"
                  >
      ...
      {isEditing && (
        ...
        >
          ...
          <TabsContent value="EXTERNAL" className="space-y-4">
            ...
            <div className="text-xs text-muted-foreground mt-4">
              Suporte nativo para YouTube e Vimeo. Conversão automática para
              iframe.
            </div>
          </TabsContent>
        </Tabs>
