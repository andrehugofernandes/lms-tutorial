    ...
    <div className="p-6 space-y-8 max-w-7xl mx-auto">
      <div className="flex flex-col gap-y-2">
        ...
        <p className="text-slate-500">
          Voce ja completou <span className="font-bold text-sky-700">{totalHoursWatched}h</span> de um total de {totalHoursTarget}h planejadas.
        </p>
      </div>
      ...
      {coursesInProgress.length > 0 && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {coursesInProgress.slice(0, 2).map((course: any) => (
        ...
        {coursesInProgress.length > 0 || completedCoursesCount > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center min-h-[30vh] text-center gap-y-4 bg-slate-50 rounded-xl border-2 border-dashed border-slate-200 p-10">
            ...
            <p className="text-muted-foreground max-w-xs">
              Comece sua jornada de aprendizado explorando nosso catalogo.
            </p>
            <Link href="/search">
