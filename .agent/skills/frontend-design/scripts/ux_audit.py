...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # Familiar patterns
        if has_form:
            has_inputs = bool(re.search(r'<input|<textarea|<select', content, re.IGNORECASE))
            if has_inputs:
                has_standard_labels = bool(re.search(r'<label|placeholder|aria-label|FormLabel|FormDescription', content, re.IGNORECASE))
